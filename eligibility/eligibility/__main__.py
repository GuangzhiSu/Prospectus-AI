...
Full four-stage pipeline::
    ...
    python -m eligibility --in data/sensetime.json --docs uploads/*.pdf \\
        --profile profile.json --feedback --out report.json
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="eligibility",
        description=(
            "Listing-eligibility diagnostic (standalone). "
            "Four stages: extraction → hard inspection → qualitative → feedback."
        ),
    )
    parser.add_argument(
        "--in",
        dest="inp",
        default=None,
        help="issuer input JSON (read-only). Optional if --docs supplied.",
    )
    parser.add_argument(
        "--docs",
        dest="docs",
        nargs="*",
        default=None,
        help="uploaded document paths for information extraction",
    )
    parser.add_argument(
        "--out",
        dest="out",
        default=None,
        help="diagnostic report JSON (default: stdout scorecard only)",
    )
    parser.add_argument(
        "--audit",
        dest="audit",
    ...
    parser.add_argument(
        ...
        dest="profile",
        default=None,
        help="run profile JSON: fx_rate_to_hkd + path_vars + deal params",
    )
    parser.add_argument(
        "--ruleset",
        dest="rulesets",
    ...
    parser.add_argument(
        ...
        dest="market",
        default="",
        help="market / board hint for extraction and feedback",
    )
    parser.add_argument(
        "--feedback",
        dest="feedback",
        action="store_true",
        help="run feedback stage (LLM or stub)",
    )
    parser.add_argument(
        "--pipeline",
        dest="pipeline",
        action="store_true",
        help="force full pipeline path (also implied by --docs / --feedback)",
    )
    parser.add_argument(
        "--auto-confirm",
        dest="auto_confirm",
        action="store_true",
        help="mark extracted quantifiable values as confirmed (trusted path)",
    )
    parser.add_argument(
        "--no-scorecard",
        dest="scorecard",
    ...
    if not args.inp and not args.docs:
        print("Provide --in issuer JSON and/or --docs uploads.", file=sys.stderr)
        return 2

    use_pipeline = bool(args.pipeline or args.docs or args.feedback)

    if use_pipeline:
        report = run_pipeline(
            issuer_path=args.inp,
            document_paths=args.docs,
            profile_path=args.profile,
            ruleset_names=args.rulesets,
            as_of_date=args.as_of,
            market_hint=args.market,
            include_feedback=bool(args.feedback or args.docs or args.pipeline),
            auto_confirm_extraction=args.auto_confirm,
        )
    else:
