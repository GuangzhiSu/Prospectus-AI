"""Assemble the diagnostic report (hard + qualitative) and optional feedback.

The hard path is evaluated here; qualitative analysis is imported from the
``qualitative`` package (never from ``hard_inspection``). Feedback is optional
and assembled by the pipeline when requested.
"""
...
from .hard_inspection.loader import gate_effective, unverified_thresholds
from .qualitative import SoftConditionEngine
...
DISCLAIMER = (
    ...
    "whether the issuer can or cannot list as a matter of law, and is not a "
    "substitute for sponsor or counsel advice. Thresholds flagged "
    "threshold_verified: false are pending a rulebook check; no gate has human "
    "sign-off for client use (human_signoff: false everywhere)."
)
...
def build_report(
    ...
    *,
    ...
    if include_qualitative:
        soft_findings = [
            asdict(f)
            for f in SoftConditionEngine().evaluate_all(
                ctx.root, narrative=narrative
            )
        ]
    ...
    return {
        ...
        "status_legend": {
            ...
            NOT_EVALUATED: (
                ...
                "or qualitative signal awaiting evidence."
            ),
        },
...
def render_scorecard(report: dict) -> str:
    """Render a plain-text, English readiness scorecard."""
    lines = []
    lines.append("Listing Eligibility Diagnostic -- readiness scorecard")
    lines.append(
        f"Issuer: {report['issuer_id']}    Generated: {report['generated_at']}"
    )
    ...
    for block in report["rulesets"]:
        tv = (
            "thresholds verified vs rulebook"
            if block["all_thresholds_verified"]
            else "thresholds not all verified"
        )
        baseline = (
            "regression" if block["in_regression_baseline"] else "not in baseline"
        )
        lines.append(
        ...
        if block.get("pending_consultation"):
            lines.append(
                f"   pending consultation: {block['pending_consultation']}"
            )
        for gate in block["gates"]:
            if gate.get("requires_llm"):
                marker = "requires_llm (qualitative)"
            elif gate["evaluated"]:
                marker = (
                    "threshold_verified"
                    if gate["threshold_verified"]
            ...
            lines.append(
                f"  [{gate['status']:13}] {gate['title']}  ({gate['rule_ref']})  "
                f"<{marker}>"
            )
    ...
    lines.append(f"Status tally: {tally}")
    pending = report["summary"]["unverified_thresholds"]
    lines.append(f"Checks pending threshold verification: {len(pending)}")
    lines.append(
        f"Human sign-off: pending on all gates ({report['summary']['gates_total']})"
    )
    lines.append("")
    lines.append(
        "Qualitative signals -- LLM / expert review layer:"
    )
    for f in report["soft_conditions"]:
        prov = (
            "rule-anchored"
            if f["provenance_verified"]
            else "rule-ref needs human verify"
        )
        signal = (
            ""
            if f["signal_level_verified"]
            else "; signal level is a heuristic probe"
        )
        lines.append(
            f"  - [{f['status']}] {f['gate_id']} (severity {f['severity']}; "
            f"{prov}{signal})  ({f['rule_ref']})"
        )
        if f.get("rationale"):
            lines.append(f"        rationale: {f['rationale']}")
        elif f.get("substantive_concern"):
            lines.append(f"        concern: {f['substantive_concern']}")
    syn = report["suitability_synthesis"]
    lines.append("")
    lines.append(
        f"Suitability synthesis (Rule 8.04): concern_level = "
        f"{syn['concern_level']}"
    )
    feedback = report.get("feedback")
    if feedback:
        lines.append("")
        lines.append(f"Feedback readiness: {feedback.get('readiness')}")
        lines.append(f"  {feedback.get('headline')}")
        for action in (feedback.get("priority_actions") or [])[:5]:
