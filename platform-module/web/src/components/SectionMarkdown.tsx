...
function createComponents(registry: PlaceholderMeta[]): Components {
  return {
    ...
    a: ({ href, children, ...rest }) => {
      const meta = getPlaceholderByHref(registry, href);
      if (meta) {
        return <PlaceholderBubble meta={meta} />;
      }
      return (
        <a
    ...
    thead: (props) => <thead className="bg-[var(--surface)]" {...props} />,
    tr: (props) => (
...
export function SectionMarkdown({
  ...
  const { text, registry } = useMemo(() => {
    const cleaned = stripModelReasoning(children);
    return preprocessProspectusMarkdown(cleaned);
  }, [children]);
  const components = useMemo(() => createComponents(registry), [registry]);
