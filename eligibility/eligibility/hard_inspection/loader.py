...
# Soft / qualitative YAML packs live under qualitative/rules/ (physically
# separated from the hard packs so the hard path never owns them).
SOFT_RULES_DIR = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "..", "qualitative", "rules")
)
...
def load_soft_layer(rules_dir: str = SOFT_RULES_DIR) -> list[dict]:
    """Load every SOFT-layer ruleset (``layer: soft``) from ``rules_dir``.

    Default directory is ``qualitative/rules/``. Soft packs may also sit under
    the hard ``rules/`` dir with ``layer: soft``; those are skipped by
    ``load_all`` and can be loaded by passing that directory here.
    """
    rulesets = []
    for path in sorted(glob.glob(os.path.join(rules_dir, "*.yaml"))):
        rs = load_ruleset(path)
