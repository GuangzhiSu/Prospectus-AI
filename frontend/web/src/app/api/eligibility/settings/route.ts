// GET/POST /api/eligibility/settings — eligibility-only inference settings
import { NextResponse } from "next/server";
import {
  readEligibilitySettings,
  writeEligibilitySettings,
  maskSettingsForClient,
  validateApiKeyInput,
  getEligibilitySettingsFilePath,
  type AppSettings,
...
export async function GET() {
  try {
    const s = await readEligibilitySettings();
    return NextResponse.json({
      ...maskSettingsForClient(s),
      settingsFile: getEligibilitySettingsFilePath(),
    });
  } catch (e) {
...
export async function POST(req: Request) {
  try {
    ...
    const next = await writeEligibilitySettings(patch);
    return NextResponse.json({
