...
_CURRENCIES = ("HKD", "RMB", "CNY", "USD", "EUR", "GBP", "SGD")
...
def _rate_to_hkd(currency: str, fx: dict | None) -> tuple[float | None, str]:
    """Look up ``currency -> HKD`` from the run-profile fx block.

    Supports the legacy single-rate shape ``{value, from_currency}`` and a
    multi-rate shape ``{rates: {RMB: 1.08, SGD: 5.85, ...}, value?, from_currency?}``.
    """
    if currency == "HKD":
        return 1.0, ""
    if not fx:
    ...
    rates = fx.get("rates")
    if isinstance(rates, dict) and currency in rates and rates[currency] is not None:
...
def _convert_to_hkd(
    ...
    """
    rate, note = _rate_to_hkd(currency, fx)
    if rate is None:
        return None, note
    if currency == "HKD":
        return value_abs, ""
...
def _to_comparable(value: float, unit: str | None, target_unit: str | None,
    ...
    if t_ccy and a_ccy and t_ccy != a_ccy:
        # Prefer converting the value into the threshold currency via HKD pivot
        # when the target is not HKD (e.g. RMB/HKD issuer vs SGD SGX thresholds).
        if t_ccy == "HKD":
            converted, note = _convert_to_hkd(a_abs, a_ccy, fx)
            if converted is None:
                return None, INDETERMINATE, note
            return converted, None, note
        a_hkd, note_a = _convert_to_hkd(a_abs, a_ccy, fx)
        t_rate, note_t = _rate_to_hkd(t_ccy, fx)
        if a_hkd is None:
            return None, INDETERMINATE, note_a
        if t_rate is None:
            return None, INDETERMINATE, note_t
        if t_rate == 0:
            return None, INDETERMINATE, f"invalid {t_ccy}->HKD rate 0"
        converted = a_hkd / t_rate
        note = f"{note_a}; then HKD->{t_ccy} at {t_rate}".strip("; ")
        return converted, None, note
...
def _eval_check_core(check: dict, ctx: EvalContext) -> CheckResult:
    ...
    if t_ccy and a_ccy and t_ccy != a_ccy:
        # Convert actual into threshold currency (HKD direct, else via HKD pivot).
        converted, conv_status, conv_note = _to_comparable(
            actual, leaf.unit, thr_unit, ctx.fx
        )
        if conv_status == INDETERMINATE or converted is None:
            return CheckResult(
                check_id, metric, INDETERMINATE, rule_ref, operator,
                _fmt_threshold(operator, threshold),
        ...
        a_abs = converted
        # Threshold stays in its own absolute units; convert threshold to same
        # currency space as a_abs (already in thr currency absolute).
        t_abs = thr_value * t_scale
