...
function sanitizePythonError(raw: string) {
  const text = (raw || "").trim();
  if (!text) return text;

  const marker = "USER_FRIENDLY:";
  ...
  // Common case: full Python traceback. Show the last meaningful line only.
  if (text.includes("Traceback (most recent call last):")) {
    const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
    const last = lines[lines.length - 1] || "Generation failed.";
    ...
    return new Promise<NextResponse>(async (resolve) => {
      ...
      proc.on("close", async (code) => {
        ...
        if (code === 0) {
          ...
          const friendly = sanitizePythonError(stderr || stdout);
          resolve(
            NextResponse.json(
              {
                ok: false,
                error: friendly || `Exit code ${code}`,
              },
              { status: 500 }
            )
          );
