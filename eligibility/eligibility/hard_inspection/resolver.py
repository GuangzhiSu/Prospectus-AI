...
def _split_selector(body: str) -> Any:
    ...
    select), a bare string key (dict key shorthand like ``[3fy]``), ``None`` for
    ``[]`` / ``[*]`` (a multi-select, unsupported for a single-leaf read), or
    ``_ABSENT`` for a malformed selector.
    """
    body = body.strip()
    if body in ("", "*"):
        return None
    if re.fullmatch(r"-?\d+", body):
        return int(body)
    if "=" not in body:
        # Bare key: financials.net_profit_positive_each_year[3fy]
    ...
    for seg in _SEG_RE.findall(path):
        if seg.startswith("["):
            ...
            if sel is _ABSENT or sel is None:
                return _ABSENT
            # Dict bare-key selector: foo[3fy] → foo["3fy"]
            if isinstance(sel, str):
                if not isinstance(cur, dict) or sel not in cur:
                    return _ABSENT
                cur = cur[sel]
                continue
